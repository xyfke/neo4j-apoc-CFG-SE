...
In our forked version of APOC, we included a new file in the https://github.com/xyfke/neo4j-apoc-CFG-SE/blob/4.4/core/src/main/java/apoc/path/DataflowPath.java[/core/src/main/java/apoc/path/Dataflow.java] called Dataflow.java to help construct the dataflow paths in behaviour alteration and publisher alteration queries. Specifically, it verifies the feasibility of the CFG paths on the fly as it construct the varWrite and parWrite paths. 

The key function that we use is: `apoc.path.varParPath(start, end, cfgCheck, hasVW, hasPW)`  
This function returns a varWrite/parWrite path from the `start` variable to the `end` variable using a BFS (Breath First Search) cfg feasibility embedded algorithm. More details can be seen in the Dataflow file.
